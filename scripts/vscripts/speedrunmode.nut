...
function IsSpeedrunModeOn(){
  //if(GetPlayer().GetWheatleyMonitorDestructionCount()<100 //session mode
  //  && GetDeveloperLevel()!=SRMODE_DEBUG //init mode
  //  && GetDeveloperLevel()!=SRMODE_INIT){ //debug mode
  //    return false
  //}
  return true
}
...
function SpeedrunModeLoad(){
  ...
  if(!IsSpeedrunModeOn())return false
  if(GetDeveloperLevel()==SRMODE_INIT)PostInit()
  ...
  switch(GetMapName()){
    case "sp_a1_intro1":
      ...
      GetPlayer().SetOrigin(Vector(-1232,4440,2857))
