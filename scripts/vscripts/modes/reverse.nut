...
if("Entities" in this) {
	...
	printl("\n=== Loading Reverse mod...");
	printl("=== Version: release-1.3, ppmod2");
	  ...
	  ppmod.addoutput(rev.auto, "OnNewGame", "!self", "RunScriptCode", "rev.spawn()");
		...
		rev.spawn <- function() {
			...
			reverse_maps["sp_a2_intro"] <- function() {
			  ...
			  rev.load <- function() {
			    ppmod.once("rev.level.fallanim()"); // Stop fall animation
			...
			reverse_maps["sp_a2_laser_over_goo"] <- function() {
			  ...
			  rev.level.ele_move <- function() {
			    ...
			    if((pos.z < 156 && speed > 0) || (pos.z > 50 && speed < 0)) {
			      elevator.SetOrigin(pos + Vector(0, 0, speed));
			      if(speed > 0) {
			        GetPlayer().SetOrigin(GetPlayer().GetOrigin() + Vector(0, 0, speed));
			...
			reverse_maps["sp_a4_intro"] <- function() {
			  ...
			  rev.loop <- function() {
			    ...
			    if(rev.level.ele_up && ppmod.trigger(994.453, 320.031, 125.531, 1119.97, 447.969, 164.221, null, null, true)){
			...
			reverse_maps["sp_a4_laser_platform"] <- function() {
			  ...
			  rev.loop <- function() {
			    ...
			    if(ppmod.trigger(2228.59, -649.417, -327.762, 2447.19, -547.978, -87.9041, "db342f763a2_ppmod")){
			      ...
			      EntFire("tbeam", "disable", "", 6.5);
			    	EntFire("rev_funnel_help", "disable", "", 7.5);
			    }
