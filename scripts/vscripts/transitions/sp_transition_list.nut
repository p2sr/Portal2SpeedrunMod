...
function OnPostTransition()
{
	...
	EntFire( "portal_stats_controller", "OnLevelStart", 5.0, 0 );
}
...
{	
	...
	try
	{
		...
		local portalGunCommand = false
		local portalGunSecondCommand = false
		local portalGunPotatoCommand = false
		local foundMap = false
		...
		// If we didn't find the map we were looking for, give the player the normal gun with both portals available
		if ( foundMap == false )
		{
			portalGunCommand = true
			portalGunSecondCommand = true
		...
		{
			printl( "=======================Trying to GivePlayerPortalgun" )
			GivePlayerPortalgun()
		}

		if (portalGunSecondCommand )
		{
			printl( "=======================Trying to UpgradePlayerPortalgun" )
			UpgradePlayerPortalgun()
		}
	
		if (portalGunPotatoCommand )
		{
			printl( "=======================Trying to UpgradePlayerPotatogun" )
			UpgradePlayerPotatogun()
		}
	}
	catch (e)
	{
		printl( "=======================Giving weapon with DLC1 not installed fall through." )
...
function TransitionFromMap()
{
	...
	{
		printl( "=======================Transitioning map with DLC1 not installed fall through." )
		RealTransitionFromMap()
	}
}
